...
# 📈 시각화 그래프 및 배당 통계
if os.path.exists("match_results.csv"):
    st.subheader("📈 체크 수 & 배당 통계 시각화")

    # 체크 수 히스토그램
    st.markdown("**체크 수 분포**")
    fig1, ax1 = plt.subplots()
    data["체크 수"].plot.hist(bins=10, ax=ax1)
    ax1.set_xlabel("체크 수")
    ax1.set_ylabel("경기 수")
    st.pyplot(fig1)

    # 배당 정보 시각화 (단순 숫자 추출 시도)
    st.markdown("**배당 정보 요약 (오버 2.5 기준)**")
    try:
        data["배당 수치"] = data["배당 정보"].str.extract(r'([0-9.]+)$').astype(float)
        fig2, ax2 = plt.subplots()
        data["배당 수치"].plot.hist(bins=10, ax=ax2)
        ax2.set_xlabel("오버 배당")
        ax2.set_ylabel("경기 수")
        st.pyplot(fig2)
    except:
        st.warning("⚠️ 배당 수치를 시각화할 수 없습니다. 형식을 확인해주세요.")
